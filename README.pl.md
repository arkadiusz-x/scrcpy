...
[Przeczytaj w innym języku](#translations)

Aplikacja ta pozwala na wyświetlanie i sterowanie urządzeniami Android podłączonymi
przez USB lub [przez TCP/IP](#tcpip-wireless). Nie wymaga dostępu do _roota_.
Działa na _GNU/Linux_, _Windows_ and _macOS_.

![screenshot](assets/screenshot-debian-600.jpg)

Koncentruje się na:

 - **lekkości**: domyśnie wyświetla tylko ekran urządzenia
 - **wydajności**: odświeżanie 30~120fps, w zależności od urządzenia
 - **jakości**: rozdzielczość 1920×1080 lub więcej
 - **niskim opóźnieniem**: [35~70ms][lowlatency]
 - **krótkim czasem uruchamiania**: po ~1 sekundzie wyświetla pierwszy obraz
 - **nieintruzyjnością**: nie pozostawia niczego zainstalowanego na urządzeniu z Androidem
 - **korzyściami dla użytkownika**: bez konta, bez reklam, bez konieczności dostępu do internetu
 - **wolnością**: wolne i otwarte oprogramowanie

[lowlatency]: https://github.com/Genymobile/scrcpy/pull/646

Jego funkcje to m.in.:
 - [nagrywanie](#recording)
 - dublowanie z [wyłączonym ekranem na urządzeniu z Androidem](#turn-screen-off)
 - [kopiowanie i wklejanie](#copy-paste) w obu kierunkach
 - [konfigurowalnie jakości](#capture-configuration)
 - Urządzenie z Androidem [jako kamerka internetowa (V4L2)](#v4l2loopback) (tylko Linux)
 - [symulowanie fizycznej klawiatury (HID)](#physical-keyboard-simulation-hid)
 - [symulowanie fizycznej myszy (HID)](#physical-mouse-simulation-hid)
 - [tryb OTG](#otg)
 - i więcej…

## Wymagania

Urządzenie z Androidem musi mieć przynajmniej API 21 (Android 5.0).

Upewnij się że [włączyłeś debugowanie adb][enable-adb] na twoim urządzeniu/urządzeniach).

[enable-adb]: https://developer.android.com/studio/command-line/adb.html#Enabling

Na niektórych urządzeniach musisz włączyć [dodatkową opcję][control]
aby sterować nim przy pomocy myszy i klawiatury.
...
Skompiluj z pilków źródłowych: [BUILD] ([proces uproszczony][BUILD_simple])
...
Dostępny jest pakiet [Snap]: [`scrcpy`][snap-link].

[snap-link]: https://snapstats.org/snaps/scrcpy

[snap]: https://en.wikipedia.org/wiki/Snappy_(package_manager)

Dla Fedory dostępny jest pakiet [COPR]: [`scrcpy`][copr-link].

[COPR]: https://fedoraproject.org/wiki/Category:Copr
[copr-link]: https://copr.fedorainfracloud.org/coprs/zeno/scrcpy/


Dla Gentoo dostępny jest [Ebuid]: [`scrcpy/`][ebuild-link].

[Ebuild]: https://wiki.gentoo.org/wiki/Ebuild
[ebuild-link]: https://github.com/maggu2810/maggu2810-overlay/tree/master/app-mobilephone/scrcpy

Możesz również [skompilować aplikację ręcznie][BUILD] ([proces uproszczony][BUILD_simple]).


### Windows

Dla Windowsa dostępne jest prekompilowane archiwum zawierające wszystkie zależnośći (włącznie z `adb`):
...
Aplikacja jest dostępna także na [Chocolatey]:
...
Możesz również [skompilować aplikację ręcznie][BUILD].


### macOS

Aplikacja jest dostępna w [Homebrew]. Zainstaluj ją:
...
Potrzebujesz `adb`, dostępne z twojej ścieżki `PATH`. Jeżeli jeszcze jego nie masz:

```bash
brew install android-platform-tools
```

Jest również dostępna w [MacPorts], które konfiguruje `adb` za ciebie:
...
Podłącz swoje urządzenie do komputera i wykonaj polecenie:

```bash
scrcpy
```

Przyjmowane są argumenty do aplikacji, których listę można uzyskać poleceniem:
